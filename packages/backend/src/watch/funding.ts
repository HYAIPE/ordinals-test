...
export function watchForFundings({
  ...
  const enqueueFunding = (funding: { address: string; id: string }) => {
    return processingQueue.add(() => checkFunding(funding));
  };
  const checkFunding = async (funding: { address: string; id: string }) => {
  ...
  const fundingMemoryMap = new Map<
    string,
  ...
  const pollForFundings$ = interval(60000 /* e.g., every 60 seconds */).pipe(
    ...
    switchMap((funding) =>
      from([funding]).pipe(
        tap((funding) => {
          fundingMemoryMap.set(funding.id, {
            ...funding,
            lastChecked: funding.lastChecked ?? new Date(),
            timesChecked: funding.timesChecked ?? 0,
          });
        }),
          ...
          return from(enqueueFunding(funding)).pipe(
            catchError((error) => {
              if (error instanceof NoVoutFound) {
                ...
                fundingMemoryMap.set(funding.id, {
                  ...funding,
                  lastChecked: now,
                  timesChecked: funding.timesChecked + 1,
                });
                return from(
                  fundingDao
            ...
            retry({
              delay(error, retryCount) {
                return of(customBackoff(retryCount + funding.timesChecked));
              },
            })
          );
        })
